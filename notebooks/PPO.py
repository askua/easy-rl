...
class Config:
    def __init__(self) -> None:
        ...
        self.new_step_api = True # 是否用gym的新api , default = False
        self.algo_name = "PPO" # 算法名字
        self.mode = "train" # train or test
        self.seed = 1 # 随机种子
        self.device = "cpu" # device to use   # cuda  | CPU
...
def plot_rewards(rewards,cfg, tag='train'):
    ...
    '''
    # sns.set()
    plt.figure(figsize=(6,6))  # 创建一个图形实例，方便同时多画几个图
...
plt.figure(figsize=(6,6))  # 创建一个图形实例，方便同时多画几个图
plt.title(f"{'train'}ing curve on {cfg.device} of {cfg.algo_name} for {cfg.env_name}")
plt.xlabel('epsiodes')
